...
scope Character {
    ...
    macro edit_action_parameters(name, action, animation, command, flags) {
        if {action} >= 0xDC && ({action} - 0xDC) * 0x14 >= Character.action_array_size.{Character.{name}_parent} {
            ...
            // Check for no parameters (sentinel offsets 0x3FE and 0x3FF indicate the action has no param entry)
            if param_offset >= 0x3FE {
    ...
    scope rare_death_fgm {
        ...
        scope check_rare_death_fgm: {
            ...
            beqzl   a0, _end                // branch if null chance
            lhu     a0, 0x00B4(t0)          // a0 = regular death FGM id
    ...
    // @ Description
    // Table of "static part" bitfields for each character
    // Uses a similar format to withheld/skipped part bitfields in character files
    // Each bit corresponds to a model part and when enabled will force it to always appear even if it's withheld/skipped
    // e.g. 0x08000000 0x00000000 = static part 4
    // Withheld part animation flags can then be used to animate the part when desired
    scope static_part {
        OS.align(8)
        table:
        constant TABLE_ORIGIN(origin())
